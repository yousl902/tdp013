...
describe("Post Routes", () => {
  ...
  describe("OPTIONS /", () => {
    it("should return 200", async () => {
      const res = await chai.request.execute(server).options("/posts/");
      const header = res.header;
      expect(header).to.have.property("allow", "POST,GET,HEAD");
      expect(header).to.not.have.property("Access-Control-Allow-Origin");
      expect(res).to.have.status(200);
    });
  });
});
